...
parser.prototype.foreignKeys = function (tableName) {
    ...
    return function (callBack) {
        underscore.each(self.loader.data[tableName].foreignKeys, function (column) {
            self.data[tableName].foreignKeys[column["COLUMN_NAME"]] = {key: column["CONSTRAINT_NAME"]};

            if (column["CONSTRAINT_NAME"] !== "PRIMARY") {
                var link = "->" + column["REFERENCED_TABLE_NAME"];
                if (column["REFERENCED_COLUMN_NAME"] !== "id") {
                    link += "." + column["REFERENCED_COLUMN_NAME"];
                }

                self.data[tableName].foreignKeys[column["COLUMN_NAME"]].extra = [link];
...
parser.prototype.convert = function (tableName) {
    var self = this,
        fields = ["field", "type", "collation", "null", "default", "key", "extra", "comment"],
        table = new Table({
            head: fields,
            chars: {
                "top": "",
